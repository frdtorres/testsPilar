...
class Tesistas extends CI_Controller
{
    ...
    public function actaBorr( $id = 0 )
    {
        ...
        */
		...
		$j1=$this->dbRepo->inDocenteEx($this->dbPilar->getOneField('tesJurados','IdJurado',"IdTram=$rowTram->Id AND Posicion=1 ORDER BY Id DESC") ); // AND Estado=20"));
		$j2=$this->dbRepo->inDocenteEx($this->dbPilar->getOneField('tesJurados','IdJurado',"IdTram=$rowTram->Id AND Posicion=2 ORDER BY Id DESC") ); // AND Estado=20"));
		$j3=$this->dbRepo->inDocenteEx($this->dbPilar->getOneField('tesJurados','IdJurado',"IdTram=$rowTram->Id AND Posicion=3 ORDER BY Id DESC") ); // AND Estado=20"));
		$j4=$this->dbRepo->inDocenteEx($this->dbPilar->getOneField('tesJurados','IdJurado',"IdTram=$rowTram->Id AND Posicion=4 ORDER BY Id DESC") ); // AND Estado=20"));
    ...
    public function actaBorrNew(){
        ...
        // Sample HTML content
        $html = '<h1>Sample Title</h1><p>This is a simple paragraph.</p>';
    ...
    public function viewCorrecs()
    {
		...
		$diasRes=mlDiasTranscHoy($rowTram->DateModif)-4;
	...
	public function choseWay($kind)
    {
		...
		if ($kind==1) {
			...
			$rowTram=$this->dbPilar->inTramTesista($sess->IdUser);
			if ($rowTram AND $rowTram->Estado == 6) {
    ...
    public function viewMensajesCuenta(){

        $this->sess->IsLoggedAccess();
        $sess = $this->sess->GetData();

        $rowMsgs=$this->dbLog->getSnapView('logCorreos', "IdUsuario='$sess->IdUser' AND TipoUser='Tesista'");


        echo "<h1>LOG DE MENSAJES</h1>";

        foreach($rowMsgs->result() as $row){
            echo "<br> <b> $row->Asunto:</b> <i>$row->Fecha</i> <br>  $row->Mensaje <hr>";
        }
